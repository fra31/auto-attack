...
def projection_linf(points_to_project, w_hyperplane, b_hyperplane):
    ...
    ub = torch.full((c2.sum(),), w.shape[1] - 1, device=device,
                    dtype=torch.float)
    ...
    if c_l.any():
        lmbd_opt = torch.clamp_min((b[c_l] - sb[c_l, -1]) / (-s[c_l, -1]),
                                   min=0).unsqueeze(-1)
        d[c_l] = (2 * a[c_l] - 1) * lmbd_opt

    lmbd_opt = torch.clamp_min((b[c2] - sb[c2, lb]) / (-s[c2, lb]),
                               min=0).unsqueeze(-1)
...
def projection_l2(points_to_project, w_hyperplane, b_hyperplane):
    ...
    s = torch.cat(((-w5.squeeze() * rs[:, 0]).unsqueeze(1),
                   torch.cumsum((-rs2 + rs) * ws,
                                dim=1) - w5 * rs[:, 0].unsqueeze(-1)), 1)
    ...
    lb = torch.zeros(c2.sum(), device=device)
    ub = torch.full((c2.sum(),), w.shape[1] - 1,
